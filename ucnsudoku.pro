...
sudoku(Rows) :-
        ...
        append(Rows, Vs), Vs ins 0..8,    % and hold digits from 0 to 8
...
ucn(UCN) :-                        % Let's get a Sudoku-compatible
    ...
    D1 * 10 + D2 #=< 31,           % up until 31,
                                   ...
                                   % as the UCN will be handpicked anyway.
    [X1,X2,X3] ins 0..8,           % Next three digits are not 9,
    X4 = 5,                        % but the last will be the broadcasted 5.
    append(SRow,[X4],UCN),         % If we remove the last digit
    all_distinct(SRow),            % all digits should be distinct.
    label(UCN),                    % Generate a UCN
    ucn_weights(Ws),               % that is valid,
    scalar_product(SRow,Ws,#=,C),  % i.e., the checksum of the first 9 digits
...
forgetOne([], [], _, _).
forgetOne([X1|L1],[X2|L2],0,J) :- !, forget(X1,X2,J), forgetOne(L1,L2,-1,J).
...
% generate a Sudoku puzzle from a solved one by deleting random elements
generateSudokuPuzzle(Attempt, Puzzle, MaxTries) :-
    generateSudokuPuzzle(Attempt, Puzzle, 0, MaxTries). % starting the count from 0

generateSudokuPuzzle(Attempt, Attempt, Tries, MaxTries) :-
    Tries > MaxTries, !,                    % If the maximum number of iterations is reached
...
generateSudokuPuzzle(Attempt, Puzzle, Tries, MaxTries) :-
    ...
    forgetOne(Attempt, NewAttempt, I, J),   % forget the number on that position
    countSolutions(NewAttempt, 1), !,       % and hope that the number of solutions is still one.
    Tries1 is Tries + 1,                    % If so, increment number of tries
    generateSudokuPuzzle(NewAttempt, Puzzle, Tries1, MaxTries). % and carry on.

generateSudokuPuzzle(Attempt, Puzzle, Tries, MaxTries) :-    % Oops, number of solutions was more than one!
    Tries1 is Tries + 1,                                     % Keep calm
    generateSudokuPuzzle(Attempt, Puzzle, Tries1, MaxTries). % and carry on.

% put everything together
generateUCNAndPuzzle(UCN, Puzzle, SolvedPuzzle) :-
    ucn(UCN),                                        % Generate a UCN,
    append(Row,[_],UCN),                             % remove last digit to create a row,
    sudokuFirstRow(Row, SolvedPuzzle),               % generate solved Sudoku and
    generateSudokuPuzzle(SolvedPuzzle, Puzzle, 100). % generate Sudoku puzzle.
